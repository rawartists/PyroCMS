...
class Files
{
    ...
    /**
     * Add a local file that wasn't in Files system to the files system
     * Needs some additional catches, as was only created to assist with the avatar manager
     *
     *
     */
    public static function add_local_file($folder_id, $type, $name, $ext = null, $mime = null, $filesize = null, $description = '', $path = false, $fullpath, $width = false,$height = false,$alt = null
    ) {
        ...
        if ($folder) {
            ...
            // We have to move the file into the system files
            $moveto = FCPATH.self::$path.$name;
           // die($fullpath.' '.$moveto);
            rename($fullpath, $moveto);
            $moveto = site_url().self::$path.$name;
            $path = $moveto;


            // Prepare data to write to files table
            ...
            if (in_array($container, $containers)) {
                ...
                if ($new_location === 'amazon-s3') {
                    ...
                    $url  = ci()->parser->parse_string(
                        Settings::get('files_s3_url'),
                        array('bucket' => $container),
                        true
                    );
...
//			$folder->name_indent		= repeater('&raquo; ', $depth) . $folder->name;
        ...
        if ($parent_id === 0) {
            foreach ($arr as $id => &$folder) {
                $folder->virtual_path = static::_buildAscSegments(
                    $id,
                    array(
                        'segments'  => $arr,
                        'separator' => '/',
